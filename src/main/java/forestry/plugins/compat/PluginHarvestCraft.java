...
public class PluginHarvestCraft extends ForestryPlugin {
    ...
    protected void registerRecipes() {
        ...
        ImmutableList<String> berries = ImmutableList
                .of("cranberry", "blackberry", "blueberry", "raspberry", "strawberry");

        ImmutableList<String> fruits = ImmutableList
                .of("pineapple", "cactusfruit", "cantaloupe", "grape", "kiwi", "chilipepper");
        ...
        ImmutableList<String> treesSpecial = ImmutableList.of("cinnamon", "maple", "paperbark", "vanillabean", "apple" // to
                                                                                                                       // prevent
                                                                                                                       // apples
                                                                                                                       // from
                                                                                                                       // getting
        ...
        ImmutableList<String> spices = ImmutableList.of("ginger", "spiceleaf", "curryleaf"
        ...
        ImmutableList<String> nuts = ImmutableList
                .of("walnut", "almond", "cashew", "chestnut", "pecan", "pistachio", "cherry" // Cherries in forestry
                                                                                             // make seed oil
                );
        ...
        for (String berryName : berries) {
            ...
            if (berry != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { berry }, Fluids.JUICE.getFluid(juiceAmount));
            }
            if (berrySeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { berrySeed }, Fluids.SEEDOIL.getFluid(seedamount));
            }
        ...
        for (String fruitName : fruits) {
            ...
            if (fruit != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { fruit }, Fluids.JUICE.getFluid(juiceAmount));
            }
            if (fruitSeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { fruitSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            }
        ...
        juiceAmount = ForestryAPI.activeMode.getIntegerSetting("squeezer.liquid.apple") / 2; // vegetables produce less
                                                                                             // juice
        juiceAmount = Math.max(juiceAmount, 1); // Produce at least 1 mb of juice.
        for (String vegetableName : vegetables) {
            ...
            if (vegetableSeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { vegetableSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            }
        ...
        for (String grainName : grains) {
            ...
            if (grain != null && wheatamount.stackSize > 0) {
                ...
                FuelManager.moistenerResource
                        .put(grain, new MoistenerFuel(grain, PluginCore.items.mouldyWheat.getItemStack(), 0, 300));
            }
            if (grainSeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { grainSeed }, Fluids.SEEDOIL.getFluid(seedamount));
        ...
        for (String treeFruitName : treeFruits) {
            ...
            Block treeFruitBlock = GameRegistry.findBlock(
                    HC,
                    "pam" + (Character.toUpperCase(treeFruitName.charAt(0)) + treeFruitName.substring(1)));
            ...
            if (treeFruit != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { treeFruit }, Fluids.JUICE.getFluid(juiceAmount));
            }
            plants.add(treeFruitName);
        }

        for (String treeName : trees) {
            Block fruitBlock = GameRegistry
                    .findBlock(HC, "pam" + (Character.toUpperCase(treeName.charAt(0)) + treeName.substring(1)));
        ...
        for (String cropName : genericCrops) {
            ...
            if (genericCropSeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { genericCropSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            }
        ...
        if (mustardCropSeed != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { mustardCropSeed }, Fluids.SEEDOIL.getFluid(seedamount));
        }
        ...
        for (String cropnutName : cropNuts) {
            ...
            if (cropnutSeed != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(10, new ItemStack[] { cropnutSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            }
            ...
            if (cropnut != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(20, new ItemStack[] { cropnut }, Fluids.SEEDOIL.getFluid(12 * seedamount));
            }
        }

        for (String nutName : nuts) {
            ItemStack nut = GameRegistry.findItemStack(HC, nutName + "Item", 1);
            Block nutBlock = GameRegistry
                    .findBlock(HC, "pam" + (Character.toUpperCase(nutName.charAt(0)) + nutName.substring(1)));
            ...
            if (nut != null) {
                RecipeManagers.squeezerManager
                        .addRecipe(20, new ItemStack[] { nut }, Fluids.SEEDOIL.getFluid(15 * seedamount));
            }
        }
        ...
        if (hcHoneyItem != null) {
            RecipeManagers.squeezerManager.addRecipe(
                    10,
                    new ItemStack[] { hcHoneyItem },
                    Fluids.HONEY.getFluid(Constants.FLUID_PER_HONEY_DROP));
        }
        ...
        if (ModUtil.isModLoaded(HCN)) {
            for (String netherCropName : genericNetherCrops) {
                ...
                if (genericNetherCropSeed != null) {
                    RecipeManagers.squeezerManager.addRecipe(
                            10,
                            new ItemStack[] { genericNetherCropSeed },
                            Fluids.SEEDOIL.getFluid(seedamount));
                }

                if (PluginManager.Module.FARMING.isEnabled() && genericNetherCropSeed != null
