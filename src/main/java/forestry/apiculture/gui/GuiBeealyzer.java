...
public class GuiBeealyzer extends GuiAlyzer {
    ...
    private void drawAnalyticsPage1(IBee bee, EnumBeeType type) {
...

        {
            ...
            drawSpeciesRow(
                    StringUtil.localize("gui.species"),
                    bee,
                    EnumBeeChromosome.SPECIES,
                    checkCustomName(customPrimaryBeeKey),
                    checkCustomName(customSecondaryBeeKey));
            textLayout.newLine();
        }
    ...
    private void drawAnalyticsPage2(IBee bee, EnumBeeType type) {
        ...
        String secondary = StringUtil.readableBoolean(
                ((AlleleBoolean) bee.getGenome().getInactiveAllele(EnumBeeChromosome.TOLERANT_FLYER)).getValue(),
                yes,
                no);
